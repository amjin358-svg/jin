...
const signalSource = [
  {
    ...
    profile: text(
      'Ultra-prime coastal scarcity, lifestyle prestige, low inventory.',
      '超高端海岸線稀缺供給，生活地位高、庫存有限。',
    ),
    score: '93%',
  },
  {
    ...
    profile: text(
      'Executive-family demand, school adjacency, strong leasing depth.',
      '企業家庭需求穩定、學區鄰近、租賃深度佳。',
    ),
    score: '88%',
  },
  {
    ...
    profile: text(
      'Education-driven family capital and larger residential footprints.',
      '教育導向家庭資本活躍，住宅坪數與庭院尺度較大。',
    ),
    score: '86%',
  },
  {
    ...
    profile: text(
      'Design-led coastal luxury with long-term brand value.',
      '以設計感與海岸生活為核心的高端住宅，長期品牌價值強。',
    ),
    score: '90%',
  },
]
...
const marketInsightsSource = [
  ...
  {
    title: text(
      'Coastal homes function as both brand and balance sheet assets',
      '海岸住宅同時是身份象徵與資產配置',
    ),
    copy: text(
  ...
  {
    title: text(
      'Execution discipline matters more than raw listing volume',
      '真正關鍵的是執行紀律，而不是單純看案量',
    ),
    copy: text(
...
function localizeWatch(language: Language): MarketWatchItem[] {
  return marketWatchSource.map((item) => ({
...
function buildContent(language: Language): SiteContent {
  const brandName = 'Formosa Pacific Advisory'
  const servicePillars = localizePillars(language)
  const marketWatch = localizeWatch(language)
  ...
  return {
    ...
    footer:
      language === 'en'
        ? 'Bilingual luxury acquisition strategy for Taiwanese investors buying Southern California property.'
        : '為台灣投資人提供雙語南加州精品不動產收購策略。',
    common: {
    ...
    home: {
      hero: {
        eyebrow:
          language === 'en'
            ? 'Bilingual acquisition platform for Taiwanese capital entering Southern California'
            : '專為台灣資本進入南加州而設計的雙語收購平台',
        title:
          language === 'en'
            ? 'Acquire prestige California real estate with institutional clarity.'
            : '用機構級判斷，配置具地位感的加州不動產。',
        highlight:
          language === 'en'
            ? 'Luxury homes, disciplined underwriting, cross-border execution.'
            : '精品住宅、嚴謹投資評估、跨境成交執行。',
        description:
          language === 'en'
        ...
        heroCardTop: {
          ...
          value:
            language === 'en'
              ? 'Luxury sourcing + cross-border execution'
              : '精品物件搜尋 + 跨境成交執行',
        },
        heroCardBottom: {
          ...
          value:
            language === 'en'
              ? 'Newport Coast, Irvine, Arcadia, Manhattan Beach'
              : 'Newport Coast、Irvine、Arcadia、Manhattan Beach',
        },
        metrics: [
          {
      ...
      servicePreview: {
        ...
        title:
          language === 'en'
            ? 'We sit between sourcing, execution, and post-close operations.'
            : '我們串接物件搜尋、交易執行與成交後管理。',
        description:
          language === 'en'
      ...
      cta: {
        ...
        title:
          language === 'en'
            ? 'Request a confidential Southern California investment briefing.'
            : '預約一場南加州不動產配置私密簡報。',
        description:
          language === 'en'
    ...
    services: {
      ...
      title:
        language === 'en'
          ? 'A premium service model for overseas buyers who need precision'
          : '專為海外買方設計的高端精準服務模式',
      description:
        language === 'en'
      ...
      advisoryHeading:
        language === 'en'
          ? 'From first inquiry in Taiwan to stabilized ownership in California.'
          : '從台灣端初次洽詢，到加州端資產穩定持有。',
      advisoryCopy:
        language === 'en'
      ...
      processTitle:
        language === 'en'
          ? 'A four-step framework built for overseas principals who value clarity.'
          : '為重視清晰度與效率的海外買方設計的四步框架。',
      processDescription:
        language === 'en'
    ...
    market: {
      ...
      title:
        language === 'en'
          ? 'Where Taiwanese investors most often find fit in Southern California'
          : '台灣投資人最常在南加州找到匹配度的區域',
      description:
        language === 'en'
    ...
    contact: {
      ...
      directDescription:
        language === 'en'
          ? 'Use the form for a tailored reply, or reach out directly for a discreet initial conversation.'
          : '可透過表單取得客製化回覆，也可直接聯繫我們，先進行低調的初步交流。',
      directItems: [
        'invest@formosapacific.com',
        language === 'en'
          ? 'Coverage: Orange County, San Gabriel Valley, South Bay, and Westside adjacencies'
          : '覆蓋市場：橘郡、San Gabriel Valley、South Bay 與西洛杉磯周邊',
        language === 'en' ? 'Languages: Mandarin Chinese and English' : '服務語言：中文與英文',
      ],
      formLabels: {
      ...
      placeholders: {
        ...
        email: language === 'en' ? 'name@example.com' : 'name@example.com',
        phone: language === 'en' ? 'Optional' : '選填',
        budget: language === 'en' ? 'e.g. USD 3M - 8M' : '例如 USD 300萬 - 800萬',
        location:
          language === 'en'
            ? 'e.g. Irvine, Newport Coast, Arcadia'
            : '例如 Irvine、Newport Coast、Arcadia',
